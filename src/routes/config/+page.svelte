  ...
  <div class="grid grid-cols-2 gap-4">
    <div>
      ...
      <ul>
        ...
        <li class="flex items-center">
          <span on:click={(_) => midi.selectInput(input)} class={`cursor-pointer px-2 ${selectedInput?.id === input.id ? 'bg-emerald-500 text-white' : 'text-gray-400'}`}>- {input.manufacturer}: {input.name}</span>
        </li>
        {/each}
      </ul>
    </div>
    ...
    <div>
      ...
      <ul>
        ...
        <li class="flex items-center">
          <span on:click={(_) => midi.selectOutput(output)} class={`cursor-pointer px-2 ${selectedOutput?.id === output.id ? 'bg-emerald-500 text-white' : 'text-gray-400'}`}>- {output.manufacturer}: {output.name}</span>
        </li>
        {/each}
      </ul>
    </div>
